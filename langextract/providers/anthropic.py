...
class AnthropicLanguageModel(base_model.BaseLanguageModel):
  ...
  def requires_fence_output(self) -> bool:
    """Anthropic supports JSON mode but may need fences for YAML."""
  ...
  def __init__(
    ...
    Args:
      model_id: The Anthropic model ID to use (e.g., 'claude-3-5-sonnet-20241022').
      api_key: API key for Anthropic service.
      base_url: Base URL for Anthropic service.
  ...
  def _process_single_prompt(
    ...
    try:
      ...
      if (v := config.get('top_k')) is not None:
        api_params['top_k'] = v
      if (v := config.get('stop_sequences')) is not None:
        api_params['stop_sequences'] = v

      response = self._client.messages.create(**api_params)

      # Extract the response text from Anthropic response format
  ...
  def infer(
    ...
    if 'stop_sequences' in merged_kwargs:
      config['stop_sequences'] = merged_kwargs['stop_sequences']
